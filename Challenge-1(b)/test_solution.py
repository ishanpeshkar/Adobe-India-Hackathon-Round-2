...
def _test_document_analyzer() -> Dict[str, Any]:
    ...
    print(f"   {ColorCodes.OKCYAN}Mock analysis created with {len(test_output['sections'])} sections{ColorCodes.ENDC}")
    return test_output

def _test_persona_processor(test_data: Dict[str, Any]) -> Dict[str, Any]:
    print(f"\n{ColorCodes.OKBLUE}Testing Persona Processor...{ColorCodes.ENDC}")
    processor = PersonaProcessor()
    user_persona = {"role": "PhD Researcher in Computational Biology"}
    task_info = {"task": "Prepare a literature review for machine learning research"}
    processed_data = processor.process_with_persona(test_data, user_persona, task_info)
    print(f"   {ColorCodes.OKGREEN}Persona type identified: {processed_data['persona_type']}{ColorCodes.ENDC}")
    print(f"   {ColorCodes.OKCYAN}Job type identified: {processed_data['job_type']}{ColorCodes.ENDC}")
    print(f"   {ColorCodes.OKCYAN}Enhanced sections: {len(processed_data['sections'])}{ColorCodes.ENDC}")
    return processed_data

def _test_section_ranker(processed_data: Dict[str, Any], user_persona: Dict[str, str], task_info: Dict[str, str]) -> List[Dict[str, Any]]:
    print(f"\n{ColorCodes.OKBLUE}Testing Section Ranker...{ColorCodes.ENDC}")
    ranker = SectionRanker()
    sorted_sections = ranker.rank_sections(processed_data['sections'], user_persona, task_info)
    print(f"   {ColorCodes.OKGREEN}Ranked sections: {len(sorted_sections)}{ColorCodes.ENDC}")
    if sorted_sections:
        print(f"   {ColorCodes.OKCYAN}Top section score: {sorted_sections[0].get('final_relevance_score', 0):.4f}{ColorCodes.ENDC}")
    return sorted_sections

def test_components():
    print(f"{ColorCodes.HEADER}Testing Individual Components{ColorCodes.ENDC}")
    print("=" * 50)
    test_data = _test_document_analyzer()
    processed_data = _test_persona_processor(test_data)
    _test_section_ranker(processed_data, {"role": "PhD Researcher in Computational Biology"}, {"task": "Prepare a literature review for machine learning research"})
    return True

# ----------------- Schema Validation -----------------

def _load_schema(schema_path: Path) -> Any:
    with open(schema_path, 'r') as file_reader:
        return json.load(file_reader)

def _validate_output_schema(output_schema: Any, result_data: Any) -> bool:
    try:
        import jsonschema
        ...
        print(f"   {ColorCodes.FAIL}Validation failed: {validation_error}{ColorCodes.ENDC}")
        return False
